...
st.markdown("過去のデータからトレンドを学習し、**向こう30日間の数値**を予測します。")
...
if scenario == "経営層：売上予測":
    ...
    if df_sku.empty:
        st.error("⚠️ エラー: 'top_sku_daily.csv' がありません。")
        st.stop()
...
if not model_df.empty:
    with st.spinner('AIが未来のパターンを計算中...'):
        ...
        fig.add_trace(go.Scatter(
            x=test['Date'], y=test[target_col], mode='lines', name='実測値 (正解)',
        ...
        if scenario == "経営層：売上予測":
            ...
            # 塗りつぶし強調（目標を超えた部分を薄い赤で塗る）
            # ※Plotlyで「特定の値以上を塗りつぶす」のは少し複雑なため、
            #   今回はシンプルに目標ラインの強調のみとします。

        else: # 物流・在庫担当の場合
            # 🧣 リスク帯（バンド）の表示
            # AI予測値の ±20% を「適正ゾーン」と定義してみる
